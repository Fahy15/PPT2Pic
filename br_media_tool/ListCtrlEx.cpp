										...
										SortType eSortBy, int nSubItem )
{
	...
	m_ctlSortHead.m_HChar.Add(strColHead);
	if (nFormat==LVCFMT_LEFT)
	{
		m_ctlSortHead.m_Format = m_ctlSortHead.m_Format + _T("0");
	}
	else if (nFormat==LVCFMT_CENTER)
	{
		m_ctlSortHead.m_Format = m_ctlSortHead.m_Format + _T("1");
	}
	else if (nFormat==LVCFMT_RIGHT)
	{
		m_ctlSortHead.m_Format = m_ctlSortHead.m_Format + _T("2");
	}
	else
	{
		m_ctlSortHead.m_Format = m_ctlSortHead.m_Format + _T("1");
...
int	CListCtrlEx::GetSelectionMark()
{
	//高亮显示默认选择的项
	int iSel = CListCtrl::GetSelectionMark();
	if (iSel >=0)
	{
...
BOOL CListCtrlEx::DeleteItem( int nItem )
{
	...
	// Modified by gaowg on 2013/1/29 20:29:15 
	// 因为删除时会导致数据混乱, 所以必须向前移数据
...
// 	}
	...
	nItem++;	
	CellDataMap::iterator itFind = m_mapCell2Data.find(make_pair(nItem, 0));
	if (itFind != m_mapCell2Data.end())
	{//不是最后一条数据, 需要把后面的数据前移
		while(itFind != m_mapCell2Data.end())
		{
			for (int nCol=0; nCol<nColCnt; ++nCol)
			{
				m_mapCell2Data[make_pair(nItem-1, nCol)] = m_mapCell2Data[make_pair(nItem, nCol)];
			}
			nItem++;
			itFind = m_mapCell2Data.find(make_pair(nItem, 0));
		}
	}
...
int CListCtrlEx::InsertItem(int nItem, LPCTSTR lpszItem, int nImage, BOOL bIntervalColor)
{
	int iIndex =  CListCtrl::InsertItem(nItem, lpszItem, nImage);
	if (bIntervalColor &&  (iIndex+1) % 2 == 0 )
	{
		SetItemColor(iIndex, LISTCTRLROWLIGHT, LISTCTRLROWLIGHTTXT);
	}
	return iIndex;
}
// public interfaces
...
void CListCtrlEx::SetCellColor(int nRow, int nCol, COLORREF bgColor, COLORREF txtColor)
{
	ASSERT(0<=nRow && nRow<GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	//FindCellData(nRow, nCol).m_bEnabled=bEnabled;
	FindCellData(nRow, nCol).SetCellColor(bgColor, txtColor);
}

void CListCtrlEx::SetItemColor(int nRow, COLORREF bgColor, COLORREF txtColor)
{
	int nCol = GetColumnCount();
	for (int i=0; i<nCol; i++)
	{
		SetCellColor(nRow, i, bgColor, txtColor);
	}
}
...
void CListCtrlEx::SetCellChecked(int nRow, int nCol, BOOL bChecked, BOOL bSended)
{
	int nColCnt=GetColumnCount();
	...
	if (CheckBox==eColType)
	{
		...
		if(GetCellRect(nRow, nCol, rcCell))
		{
			...
			// send message to parent
			CWnd *pWnd=NULL;
			if ((pWnd = GetParent())!=NULL && bSended)
			{
...
//设置第一行为选中状态
BOOL CListCtrlEx::SetCheck(_In_ int nItem, _In_ BOOL fCheck /* = TRUE */)
{
	SetCellChecked(nItem, 0, fCheck);
	return TRUE;
	//return CListCtrl::SetCheck(nItem, fCheck);
}
BOOL CListCtrlEx::GetCheck(_In_ int nItem)
{
	if ( GetCellChecked(nItem, 0) > 0 )
		return TRUE;
	return FALSE;
										...
										int nRow, int nCol, LRESULT *pResult)
{
	...
	if (m_ctlSortHead.m_Format[nCol]=='0')
	{
		nFormat = DT_LEFT;
	}
	else if (m_ctlSortHead.m_Format[nCol]=='1')
	{
		nFormat = DT_CENTER;
	}
	else if (m_ctlSortHead.m_Format[nCol]=='2')
	{
		nFormat = DT_RIGHT;
	}
	...
	case Normal:
		*pResult |= CDRF_SKIPDEFAULT;
		DrawNormal(pDC, strText, rcCell, bSelected, FindCellData(nRow, nCol), nFormat);
		return;
	case CheckBox:
		*pResult |= CDRF_SKIPDEFAULT;
...
void CListCtrlEx::DrawNormal(CDC *pDC, CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData, int uLvcFmt)
{
	...
	COLORREF ntxtColor;
	if (cellData.m_nTxtColor >=0)
	{
		ntxtColor = cellData.m_nTxtColor;
	}else
	{
	...
	COLORREF nBgColor;
	if (cellData.m_nBgColor >=0)
	{
		nBgColor = cellData.m_nBgColor;
	}else
	{
		nBgColor =GetSysColor(COLOR_WINDOW);
	}
	COLORREF crText=bSelected?GetSysColor(COLOR_HIGHLIGHTTEXT):ntxtColor;
	COLORREF crTextBkgrnd=bSelected?GetSysColor(COLOR_HIGHLIGHT):nBgColor;
	// fill background
	...
	pDC->DrawText(strText, &rcText, uLvcFmt|DT_VCENTER|DT_SINGLELINE);
...
void CListCtrlEx::DrawCheckBox(CDC *pDC,  CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData)
{
	...
	COLORREF crText=bSelected?GetSysColor(COLOR_HIGHLIGHTTEXT):ntxtColor;
	COLORREF crTextBkgrnd=bSelected?GetSysColor(COLOR_HIGHLIGHT):nBgColor;
	COLORREF crCheckFrame=cellData.GetEnable()?RGB(0,0,0):RGB(192,192,192);
	COLORREF crCheckMark=cellData.GetEnable()?RGB(0,0,0):RGB(192,192,192);
	//------------calculate the check box & Text rect ---------------------/
	CRect rcText(rcCell);
	CRect rcCheck(rcCell);
	if (rcCell.Height() > 20)
	{//高度过大时, checkbox离边框大一些
		rcCheck.bottom -= 5;
		rcCheck.top+=5;		
	}else
	{//高度正常时, checkbox离边框小一些
		rcCheck.bottom -= 2;
		rcCheck.top+=2;
	}
	rcCheck.left += 7;        
...
void CListCtrlEx::ShowInPlaceCombo(CellIndex ix)
{
	if (ix.first>=0 && ix.first<GetItemCount() && ix.second>=0 && ix.second< GetColumnCount())
	{
		...
		{	
			...
			rcCell.bottom = rcCell.top + 100; //增加下拉高度
...
void ListCtrlEx::CListCtrlEx::OnLvnEndlabeledit(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	//发送消息时,如果返回>0 则表示设置失败
	if (GetParent()->SendMessage(WM_VALIDATE, GetDlgCtrlID(), (LPARAM)pDispInfo) ==0)
	{
		SetItemText(pDispInfo->item.iItem, pDispInfo->item.iSubItem, pDispInfo->item.pszText);
		
		CWnd* pWnd = GetParent();
		if(pWnd)
			pWnd->SendMessage(WM_ListCtrlEx_COMBOX_CHANGED, GetParent()->GetDlgCtrlID(), (LPARAM)pDispInfo);
	}
	*pResult = 0;
}
...
{
	m_ctlSortHead.m_Height = Height;
}
void CListCtrlEx::SetRowHeigt(int nHeight) //高置行高
{
	//用图片撑高
	CImageList imagelist;
	imagelist.Create(1, nHeight, ILC_COLOR, 1, 1);  
	SetImageList(&imagelist, LVSIL_SMALL); 
}
void CListCtrlEx::SetHeaderGradientColor(ARGB clrBegin, ARGB clrEnd)
{
	m_ctlSortHead.SetGradientColor(clrBegin, clrEnd);
}

void CListCtrlEx::SetSortItems(int nSubItem, BOOL bIsAsc)
{
	if(IsSupportSort())
	{
		vector<DWORD_PTR> vecItemDatas(GetItemCount());
		...
		PreSortItems(vecItemDatas);
		//SortType eSortBy=GetColumnSortBy(pNMLV->iSubItem);
		//BOOL bIsAsc=!IsColumnSortAsc(nSubItem);
		SetColumnSortDirection(nSubItem, bIsAsc);
		_SortParam_t aSortParam;
		aSortParam.m_ColParam=m_mapCol2Sort[nSubItem];
		aSortParam.m_nSortCol=nSubItem;
		aSortParam.m_pTheList=this;
		SortItems(CompareFunc, (DWORD_PTR)&aSortParam);
		m_ctlSortHead.SetSortArrow(nSubItem, !bIsAsc);
		PostSortItems(vecItemDatas);
	}
}