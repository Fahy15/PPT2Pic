...
namespace ListCtrlEx
{
	...
	const DWORD LISTCTRLHEADCOLOR_BEGIN = Color::MakeARGB(255, 210, 240, 255);
	const DWORD LISTCTRLHEADCOLOR_END = Color::MakeARGB(255, 255, 255, 255);
	const UINT	LISTCTRLROWHEIGHT = 22;	//默认行高
	const float	LISTCTRLHEADHEIGHT = 1.5;	//默认表头高倍数
	const COLORREF LISTCTRLROWLIGHT = RGB(248,248,248); //设置表格变换颜色1
	const COLORREF LISTCTRLROWLIGHTTXT = RGB(0,0,0);
    ...
    #define WM_ListCtrlEx_COMBOX_CHANGED WM_USER+8010
    #define WM_ListCtrlEx_CHECKBOX_CHANGED
	...
	public:
		...
		public:
			...
			void SetCellColor(COLORREF bgColor, COLORREF txtColor)
			{
...
// overrides
	public:
		...
		int InsertItem(int nItem, LPCTSTR lpszItem, int nImage=0, BOOL bIntervalColor=TRUE);

// operations
	public:
		//add by gaowg 2013-1-28
		//修改checkbox在设置选择时无法显示选中状态的问题
		//使之符合正常的checbox类型
		BOOL SetCheck(_In_ int nItem, _In_ BOOL fCheck  = TRUE); //设置第一行为选中状态
		BOOL GetCheck(_In_ int nItem);
		//////////////////////////////////////////////////////////////////////////
		...
		void				SetCellColor(int nRow, int nCol, COLORREF bgColor, COLORREF txtColor);
		void				SetItemColor(int nRow, COLORREF bgColor, COLORREF txtColor);
		...
		void				SetCellChecked(int nRow, int nCol, BOOL bChecked=TRUE, BOOL bSended = TRUE);
		// for radio only
		...
		//设置表头渐变颜色
		void				SetHeaderGradientColor(ARGB clrBegin, ARGB clrEnd);

		//设置排序 nSubItem从0列开始
		void                SetSortItems(int nSubItem,BOOL bIsAsc);
	protected:
